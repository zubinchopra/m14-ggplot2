...
column.names <- colnames(diamonds)
number.of.rows <- nrow(diamonds)
...
ggplot(data = diamonds.sample) + geom_point(mapping = aes(x = price, y = carat, color = cut), alpha = 0.1) + geom_smooth(mapping = aes(x = price, y = carat, color = cut), se = F)
